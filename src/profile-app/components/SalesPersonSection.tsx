...
type SalesPersonSectionProps = {
  sectionName?: string
}

export const SalesPersonSection = ({ sectionName = 'Sales Person' }: SalesPersonSectionProps) => {
  const { cardOwnerId, design } = useProfileDisplay()
  const profileId = cardOwnerId?.trim() ?? ''
  const resolvedSectionName = sectionName.trim() || 'Sales Person'
