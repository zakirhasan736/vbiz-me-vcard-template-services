...
function resolveEventImage(item: DynamicPostListItem): string {
...
function EventCard({ item, idx, accent }: { item: DynamicPostListItem; idx: number; accent: string }) {
  const imageUrl = resolveEventImage(item)
  ...
  const card = (
    <motion.article
      ...
      className={`group relative flex h-full flex-col overflow-hidden rounded-3xl border border-zinc-200 bg-white/50 shadow-sm backdrop-blur-xl transition-colors hover:bg-white/80 dark:border-zinc-800/80 dark:bg-zinc-900/50 dark:hover:bg-zinc-900/80${detailUrl ? 'cursor-pointer' : ''}`}
    >
      {imageUrl ? (
        ...
        <div className="flex h-40 items-center justify-center bg-zinc-100 dark:bg-zinc-950">
          <Ticket size={40} style={{ color: accent }} />
        </div>
      )}

      <div className="flex flex-1 flex-col p-6">
        {eventDate ? (
          <div className="mb-3 inline-flex items-center gap-2 text-xs font-bold tracking-wider text-zinc-500 uppercase dark:text-zinc-400">
...
export const EventsSection = () => {
  ...
  const { data, isLoading, isError } = useGetEventsQuery(profileId, { skip: !profileId })

  const sectionTitle = data?.sectionTitle ?? 'Events'
  const items = data?.posts ?? []
  const showInitialLoader = isLoading && items.length === 0
  const showEmptyState = !isLoading && !isError && items.length === 0

  if (!profileId) return null
  if (showInitialLoader) return <EventsSkeleton />
      ...
      <div className="w-full pb-20">
        <div className="flex min-h-[320px] flex-col items-center justify-center rounded-3xl border border-dashed border-zinc-200 bg-white/40 p-10 text-center dark:border-zinc-800/80 dark:bg-zinc-900/30">
          ...
          <p className="max-w-md text-sm leading-relaxed font-medium text-zinc-600 dark:text-zinc-400">
            No events have been published yet. Add content from the vCard editor Events tab.
          </p>
        </div>
      </div>
    )
  ...
  return (
    <div className="w-full pb-20">
      ...
      <div className="relative z-20 mt-4 grid grid-cols-1 gap-4 md:grid-cols-2">
        {items.map((item, idx) => (
          <EventCard key={item.id} item={item} idx={idx} accent={accent} />
        ))}
      </div>
    </div>
  )
